...
namespace WCPS.Class
{
    public class SendSMS
    {

        public static string url = "http://bulksms.2way.co.za:5567/eapi/submission/send_sms/2/2.0";
        ...
        {
            string sms_result = Post(url, data).TrimEnd(System.Environment.NewLine.ToCharArray());

            Hashtable result_hash = new Hashtable();

            string tmp = "";
            tmp += DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss.fff") + " (SMS Server Result) Response from server: " + sms_result + Environment.NewLine;
            string[] parts = sms_result.Split('|');
            //string tmStamp = DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss.fff");
            //File.AppendAllText(Constants.Path, tmStamp + " " + tmp + Environment.NewLine);
            ...
            if (parts.Length != 3)
            {
                tmp += DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss.fff") + " (SMS Server Failed) Error: could not parse valid return data from server." + Environment.NewLine;
                //tmStamp = DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss.fff");
                //File.AppendAllText(Constants.Path, tmStamp + " " + tmp + Environment.NewLine);
            }
            else
            {
                if (statusCode.Equals("0"))
                {
                    ...
                    tmp += DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss.fff") + " (SMS Server Sent) Message sent - batch ID " + parts[2] + Environment.NewLine;
                }
                else if (statusCode.Equals("1"))
                {
                    ...
                    result_hash.Add("success", 1);
                    result_hash.Add("api_batch_id", parts[2]);
                    tmp += DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss.fff") + " (SMS Server Sent) Message sending later - batch ID " + parts[2] + Environment.NewLine;
                }
                else
                {
                    result_hash.Add("success", 0);
                    tmp += DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss.fff") + " (SMS Server Failed) Error sending: status code " + parts[0] + " description: " + parts[1] + Environment.NewLine;
                }
            }
